...
if type docker > /dev/null 2>&1 && type dockerd > /dev/null 2>&1; then
    ...
    if [ "${USE_MOBY}" = "true" ]; then
        ...
        echo "Finished installing Moby CLI."
    else
...
DOCKER_OWNER="root"
if [ "${ENABLE_NONROOT_DOCKER}" = "true" ] ; then DOCKER_OWNER=${USERNAME} ; fi

chmod +x /usr/local/share/docker-init.sh
chown ${DOCKER_OWNER}:root /usr/local/share/docker-init.sh
chown ${DOCKER_OWNER}:docker /usr/var/docker.sock

echo "Docker/Docker-Compose/Moby CLI setup is complete."
